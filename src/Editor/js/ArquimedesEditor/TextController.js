...
var descartesJS = (function(descartesJS) {
   ...
   *
   */
  descartesJS.TextController = function(parent, container, canvas, textNodes, defaultStyle, externalColor) {
    var self = this;
    this.parent = parent;
    ...
    this.converter = new descartesJS.TextConverter();
    ...
    this.range = new descartesJS.Range(container);
    ...
    this.caret = new descartesJS.Caret(container, externalColor, true);
    this.caret.set(textNodes.getFirstTextNode(), 0);

    this.startCaret = new descartesJS.Caret(container, externalColor);
    this.startCaret.set(this.caret.node, this.caret.offset);
    this.startCaret.hide();

    this.updateTextfield();

    this.undoRedoManager = new descartesJS.UndoRedoManager(textNodes, self.caret, self.startCaret, self.parent);
...
// console.log(evt.key, evt.which)
      ...
      ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        var line = node.parent;
        if (line.nodeType === "textLineBlock") {
          var splitRight = new descartesJS.TextNode(node.value.substring(offset), "text", node.style);

          node.value = node.value.substring(0, offset);

          var newLine = new descartesJS.TextNode("", "textLineBlock", line.style);
      ...
      // undo
        ...
        if (state) {
          ...
          positionCaretAux(state.caretX, state.caretY, caretLineAux(state.caretX, state.caretY, self.textNodes.querySelectorAll("textLineBlock")));
          self.startCaret.set(self.caret.node, self.caret.offset);
    ...
    container.addEventListener("paste", function(evt) {
      ...
      if (data) {
        var nodes = descartesJS.JSONtoTextNodes(JSON.parse(data), self.parent.evaluator);
        ...
        if (nodes.nodeType === "text") {
          ...
          // if the text node has not the same style, then split the caret node and insert the new text node
            ...
            var rightNode = new descartesJS.TextNode(rightText, "text", self.caret.node.style.clone());
        ...
        // nodes.nodeType === "CONTAINER"
          ...
          if (nodes.querySelectorAll("textLineBlock").length > 0) {
            ...
            caretParent.insertAfter(self.caret.node, rightNode);

            var tmpParent = new descartesJS.TextNode("", caretParent.nodeType, caretParent.style.clone());
          ...
          // various nodes in a single line
            ...
            if ((nodes.insideFormula) && (!inFormula(self.caret.node)))  {
              var tmpFormula = new descartesJS.TextNode("", "formula", self.caret.node.style.clone());
      ...
      self.undoRedoManager.put(self.textNodes);
    });
    ...
    function copyAux(evt) {
      ...
      if (self.startCaret.node === self.caret.node) {
        ...
        var newNode = new descartesJS.TextNode(textSelected, "text" , self.caret.node.style.clone());

        evt.clipboardData.setData("text/plain", textSelected);
        evt.clipboardData.setData("text/richText", newNode.stringify());
        // evt.clipboardData.setData("text/richText", JSON.stringify(newNode, descartesJS.stringifyAux));
      }
      ...
      // the selection is in differents nodes
        ...
        var rightTextNode = new descartesJS.TextNode(rightTextStartCaret, "text", startCaret.node.style.clone());
        
        var leftTextEndCaret = endCaret.node.value.substring(0,  endCaret.offset);
        var leftTextNode = new descartesJS.TextNode(leftTextEndCaret, "text", endCaret.node.style.clone());

        var newNode = new descartesJS.TextNode("", "CONTAINER", "");
        ...
        while (next !== endCaret.node) {
          ...
          if (next) {
            ...
            if (nextLine) {
              ...
              if (nextLine) {
                lastLine = new descartesJS.TextNode("", nextLine.nodeType, nextLine.style.clone());
    ...
    function positionCaretAux(x, y, texts) {
      ...
      descartesJS.auxCtx.font = textNode.style.toString(); // set the style of the canvas context only one time, to check te width of the text
      ...
      for (var i=0, l=textNode.value.length; i<l; i++) {
        ...
        charWidth = parseInt(0.5 + descartesJS.auxCtx.measureText(textNode.value.substring(i, i+1)).width);
   ...
   * 
   */
  descartesJS.TextController.prototype.changeStyle = function(prop, value) {
    var self = this;
    ...
    if (self.startCaret.node === self.caret.node) {
      ...
      if (inFormula(self.caret.node) && (prop === "size")) {
      ...
      // change the property freely
        ...
        var newNode = new descartesJS.TextNode(self.caret.node.value.substring(startOffset, endOffset), "text" , newStyle);
    ...
    // the selection is in differents nodes
      ...
      var startCaretRightNode = new descartesJS.TextNode(rightTextStartCaret, "text", startCaret.node.style.clone());

      endCaret.node.value = rightTextEndCaret;
      var endCaretLeftNode = new descartesJS.TextNode(leftTextEndCaret, "text", endCaret.node.style.clone());
   ...
   * 
   */
  descartesJS.TextController.prototype.addNode = function(type, extra) {
    var self = this;
    var textNodes = self.textNodes;
    var externalColor = self.externalColor;
    ...
    if (type === "formula") {
      if (extra) {
        tmpStyle.textItalic = true;
        newNode = new descartesJS.TextNode("", type, tmpStyle);
      }
      else {
        newNode = new descartesJS.TextNode("", type, tmpStyle);
      }
      ...
      var numNode = new descartesJS.TextNode("", "numerator", tmpStyle);
      var denNode = new descartesJS.TextNode("", "denominator", tmpStyle);
      ...
      var indexNode = new descartesJS.TextNode("", "index", tmpStyle);
      var radicandNode = new descartesJS.TextNode("", "radicand", tmpStyle);
      ...
      var fromNode = new descartesJS.TextNode("", "from", tmpStyle);
      var toNode = new descartesJS.TextNode("", "to", tmpStyle);
      var whatNode = new descartesJS.TextNode("", "what", tmpStyle);
      ...
      for (var i=0, l=newNode.rows*newNode.columns; i<l; i++) {
        var elementNode = new descartesJS.TextNode("", "element", tmpStyle);
    ...
    if ( (!inFormula(self.caret.node)) && (type !== "formula") && (type !== "text")) {
      var formulaNode = new descartesJS.TextNode("", "formula" , tmpStyle);
   ...
   * 
   */
  descartesJS.TextController.prototype.removeSelection = function() {
    var self = this;
    var textNodes = self.textNodes;
    var externalColor = self.externalColor;
   ...
   * 
   */
  descartesJS.TextController.prototype.updateTextfield = function() {
    var self = this;
   ...
   * 
   */
  descartesJS.TextController.prototype.getTextNodes = function() {
    return this.textNodes;
  }
  descartesJS.TextController.prototype.setNewNodes = function(nodes, color) {
  ...
  descartesJS.TextController.prototype.notifyParentStyle = function() {
    if (this.toolbar) {
      this.toolbar.changeStyle(this.caret.node.style);
    }
  }
  descartesJS.TextController.prototype.insertSymbol = function(symbol) {
  ...
  descartesJS.TextController.prototype.addFormula = function() {
    this.addNode("formula", true);
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addDynamicTextNode = function() {
    this.addNode("dynamicText");
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addFraction = function() {
    this.addNode("fraction");
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addSuperIndex = function() {
    this.addNode("superIndex");
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addSubIndexNode = function() {
    this.addNode("subIndex");
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addRadicalNode = function() {
    this.addNode("radical");
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addSumNode = function() {
    this.addNode("sum");
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addIntegralNode = function() {
    this.addNode("integral");
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addLimitNode = function() {
    this.addNode("limit");
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addMatrixNode = function(rows, cols) {
    this.addNode("matrix", {rows: rows, cols: cols});
    this.textfield.focus();
  }
  descartesJS.TextController.prototype.addDefpartsNode = function(parts) {
    this.addNode("defparts", {parts: parts});
    this.textfield.focus();
  }

  // update the cursor position, and the added text
  descartesJS.TextController.prototype.updateScreen =  function() {
    this.textNodes.update(0, 0, 20, false, "justify", "left", "000000", this.parent.w);
    this.parent.setArquimedesHeight();
    this.range.clear();
  }
