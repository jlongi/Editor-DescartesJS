...
var richTextEditor = (function(richTextEditor) {
   ...
   *
   */
  richTextEditor.TextController = function(component, defaultStyle, externalColor, win) {
    ...
    this.textBlock.window = this.window;
    ...
    if (selection.rangeCount > 0) {
      ...
      if ((start.nodeType !== 3) && (start.getAttribute("class") === "TextLine") && (!start.hasChildNodes())) {
        var newTextNode = self.window.document.createElement("span");
    ...
    this.textBlock.addEventListener("copy", function(evt) {
      ...
      var tmpDiv = self.window.document.createElement("div");
    ...
    this.textBlock.addEventListener("paste", function(evt) {
      ...
      // a text tree
        ...
        // text in a single line copied
          ...
          var docFra = self.window.document.createDocumentFragment();
    ...
    this.textBlock.addEventListener("mousedown", function(evt) {
      ...
      var tmpRange = self.window.document.caretRangeFromPoint(evt.clientX, evt.clientY);
      selection.removeAllRanges();
     ...
     *
     */
    this.window.document.addEventListener("mouseup", function(evt) {
    ...
    this.textBlock.addEventListener("keypress", function(evt) {
      ...
      if (range.startContainer.nodeType !== 3) {
        ...
        if ( (container.getAttribute) && (container.getAttribute("class")) && (container.getAttribute("class").match("TextNode")) ) {
          var newTextNode = self.window.document.createTextNode(char);
...
// console.log(commonAncestor);
    ...
    if (commonAncestor.nodeType === 3) {
      ...
      // outside a formula split the text and add the pieces to the text line
        ...
        var docFra = this.window.document.createDocumentFragment();
...
// console.log("hola", newContainer)
  ...
  richTextEditor.TextController.prototype.createDynamicTextNodeDialog = function() {
    ...
    this.dynamicTextNodeDialog = self.window.document.createElement("dialog");
    this.dynamicTextNodeDialog.style.padding = "10px";
    var form_div = self.window.document.createElement("div");
    var decimals_label = self.window.document.createElement("label");
    decimals_label.innerHTML = "decimales";
    this.decimals_inpt = self.window.document.createElement("input");
    this.decimals_inpt.setAttribute("type", "text");
    var fixed_id = "fixed_id_" + parseInt(Math.random()*1000);
    var fixed_label = self.window.document.createElement("label");
    fixed_label.innerHTML = "fijo";
    fixed_label.setAttribute("for", fixed_id);
    this.fixed_inpt = self.window.document.createElement("input");
    this.fixed_inpt.setAttribute("type", "checkbox");
    this.fixed_inpt.setAttribute("id", fixed_id)
    var exprValue_label = self.window.document.createElement("label");
    exprValue_label.innerHTML = "valor";
    this.exprValue_inpt = self.window.document.createElement("input");
    ...
    form_div.appendChild(window.document.createElement("br"));
    form_div.appendChild(exprValue_label);
    form_div.appendChild(this.exprValue_inpt);

    var btn_div = self.window.document.createElement("div");
    var btn_accept = self.window.document.createElement("button");
    btn_accept.setAttribute("id", "btn_accept_code_editor");
    btn_accept.innerHTML = "ace";
    var btn_cancel = self.window.document.createElement("button");
    ...
    this.window.document.body.appendChild(this.dynamicTextNodeDialog);
  ...
  richTextEditor.TextController.prototype.createMatrixDialog = function() {
    ...
    this.matrixDialog = self.window.document.createElement("dialog");
    this.matrixDialog.style.padding = "10px";
    var form_div = self.window.document.createElement("div");
    var columns_label = self.window.document.createElement("label"); // m
    columns_label.innerHTML = "m";
    this.columns_inpt = self.window.document.createElement("input");
    this.columns_inpt.setAttribute("type", "text");
    this.columns_inpt.setAttribute("style", "width:80px;");
    this.columns_inpt.value = 2;
    var rows_label = self.window.document.createElement("label"); //n
    rows_label.innerHTML = "n";
    this.rows_inpt = self.window.document.createElement("input");
    ...
    this.window.document.body.appendChild(this.matrixDialog);
  ...
  richTextEditor.TextController.prototype.createCasesDialog = function() {
    ...
    this.casesDialog = self.window.document.createElement("dialog");
    this.casesDialog.style.padding = "10px";
    var form_div = self.window.document.createElement("div");
    var parts_label = self.window.document.createElement("label"); // m
    parts_label.innerHTML = "partes";
    this.parts_inpt = self.window.document.createElement("input");
    ...
    self.window.document.body.appendChild(this.casesDialog);
